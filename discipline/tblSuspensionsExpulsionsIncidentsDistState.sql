if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[tblSuspensionsExpulsionsIncidentsDistState]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[tblSuspensionsExpulsionsIncidentsDistState]
GO
...
into tblSuspensionsExpulsionsIncidentsDistState
...
CREATE  CLUSTERED  INDEX [AllClustered] ON [dbo].[tblSuspensionsExpulsionsIncidentsDistState]([Year], [Fullkey], [Schooltype], [Grade], [Race], [Gender], [Disability], [IncidentsSuspWeaponDrugYes], [IncidentsSuspWeaponDrugNo], [IncidentsExpWeaponDrugYes], [IncidentsExpWeaponDrugNo], [IncidentsSuspDurationLong], [IncidentsSuspDurationShort], [IncidentsExpDurationTotal], [IncidentsTotalWeaponDrugYes], [IncidentsTotalWeaponDrugNo]) WITH  FILLFACTOR = 100 ON [PRIMARY]
GO

GRANT  SELECT  ON [dbo].[tblSuspensionsExpulsionsIncidentsDistState]  TO [netwisco]
GO
