...
--sum([FTE]) as 'FTE',
sum(
  CASE WHEN (LinkSubject = 'SOC' AND ESEAHighlyQualified <> 'y') THEN 0
  ELSE [FTE] END
      ) as 'FTE',
...
sum(case [ESEAHighlyQualified] when 'Y' then cast([FTE] as decimal(5,2)) else 0 end) as 'FTEEHQYes',
sum(case [ESEAHighlyQualified] when 'N' then cast([FTE] as decimal(5,2)) else 0 end) as 'FTEEHQNo',
...
group by [wisconsin].[dbo].[tblTQAssBeta].[Year], agency.agencykey, agency.FullKey, [tblTQAssBeta].[StaffKey], agency.District,
--agency.School,
